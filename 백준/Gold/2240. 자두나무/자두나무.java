...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[][][] dp = new int[T + 1][3][W + 1]; // 자두가 떨어지는 위치 | 이동 횟수 | 위치

        int tree = Integer.parseInt(br.readLine());

        if (tree == 1) {
            // 처음에 1번 나무에서 떨어진다면?
            dp[1][1][0] = 1; // 자두도 1번에서 시작
        } else {
            // 처음 2번 나무에서 떨어진다면?
            dp[1][2][1] = 1; // 자두는 2번으로 이동
        }
        ...
        for (int i = 2; i < T + 1; i++) {
            ...
            if (tree == 1) { // 자두가 1번 나무에서 떨어진다면
                // 여태 이동을 1번도 안 했을 때,
                // 현재 나의 위치가 1번일 때,
                dp[i][1][0] = dp[i - 1][1][0] + 1; // 자두를 얻음
                // 현재 나의 위치가 2번일 때,
                dp[i][2][0] = dp[i - 1][2][0]; // 자두를 얻지 못함
                ...
                // 여태 이동을 1번도 안 했을 때,
                // 현재 나의 위치가 1번일 때,
                dp[i][1][0] = dp[i - 1][1][0]; // 자두를 얻지 못함
                // 현재 나의 위치가 2번일 때
                dp[i][2][0] = dp[i - 1][2][0] + 1; // 자두를 얻음
